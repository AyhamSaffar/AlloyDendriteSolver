...
namespace alloys
{
    /// @brief contains physical alloy system constants required for the LGK model. Concentration units (C%) can either
    ...
    struct LGKAlloy
    {
        ...
        LGKAlloy(double L, double Cp, double m, double k0, double r, double D, double a, double o);
    };

    /// @brief contains physical alloy system constants required for the BCT and LGK models. Concentration units
    /// (C%) must be in atom percent.
    struct BCTAlloy: public LGKAlloy
    {
        double a0{};    // Solid atomic spacing - m
        double V0{};    // speed of sound in liquid - m/s
        double Tm{};    // Pure solid melting point - K

        BCTAlloy(double L, double Cp, double m, double k0, double r, double D, double a, double o,
            double a0, double V0, double Tm
        );
    };
    
    /// @brief contains physical alloy system constants required for the dyanmic BCT, BCT or LGK models. Concentration
    /// units (C%) must be in atom percent.
    class DynamicAlloy: public BCTAlloy
    {
        public:
            DynamicAlloy(
                const BCTAlloy& alloy, double DA0, double DEa, std::vector<double> TlFit, std::vector<double> k0Fit
            );
            // double DAtT(double T);


        private:
            double m_DA0{}; // Arrhenius constant of diffusivity - m2/s
            double m_DEa{}; // activation energy for diffusion - J/mol
            std::vector<double> m_TlFit{}; // polynomial fit of Tl for a given C0 (0th to nth order coefficient)
            std::vector<double> m_k0Fit{}; // polynomial fit of k0 for a given T (0th to nth order coefficient)
    };
    
}

/// @brief create a new LGKAlloy object.
...
inline alloys::LGKAlloy::LGKAlloy(double L, double Cp, double m, double k0, double r, double D, double a, double o)
    :L{L}, Cp{Cp}, m{m}, k0{k0}, r{r}, D{D}, a{a}, o{o} {}

/// @brief create a new BCTAlloy object.
/// @param L Fusion enthalpy - J/mol
/// @param Cp Melt heat capacity - J/(mol K)
/// @param m Equilibrium liquidus slope - K/at.%
/// @param k0 Partition coefficient - at.% / at.%
/// @param r Gibbs-Thomson coefficient - K m
/// @param D Diffusion coefficient of solute in liquid - m2/s
/// @param a Thermal diffusivity of liquid - m2/s
/// @param o Stability constant - m/m
/// @param a0 Solid atomic spacing - m
/// @param V0 speed of sound in liquid - m/s
/// @param Tm Pure solid melting point - K
inline alloys::BCTAlloy::BCTAlloy(
    double L, double Cp, double m, double k0, double r, double D, double a, double o, double a0, double V0, double Tm
): LGKAlloy{L, Cp, m, k0, r, D, a, o}, a0{a0}, V0{V0}, Tm{Tm} {}
...
inline alloys::DynamicAlloy::DynamicAlloy(
    const BCTAlloy& alloy, double DA0, double DEa, std::vector<double> TlFit, std::vector<double> k0Fit
): BCTAlloy{alloy}, m_DA0{DA0}, m_DEa{DEa}, m_TlFit{TlFit}, m_k0Fit{k0Fit} {}
...
namespace alloys
{
    ...
    const BCTAlloy NiB2007_atp{1.72e4, 36.39, -16.3, 3.45e-2, 3.42e-7, 3e-9, 8.5e-6, o, (3e-9)/18.9, 425, 1726};
    ...
    const BCTAlloy NiB1997_atp{1.72e4, 36.39, -14.3, 8e-6, 0.464/NiS, 2.42e-9, 1e-5, o, (2.42e-9)/7.6, 2e3, 1726};
    ...
    const LGKAlloy NiSn_wtp{1.5e5*NiAr, 500*NiAr, -16.2, 0.61, 0.25/8.4e5, 5e-9, 5e-6, o};
    ...
    const LGKAlloy AlFe_wtp{971e6*AlAr/AlMeltDensity, 2.67e6*AlAr/AlMeltDensity, -3.7, 0.038, 1e-7, 2e-9, 0.34e-4, o};
    ...
    const BCTAlloy AgCu_wtp{11'900, 31.8, -8.0, 0.37, 1.53e-7, 2.1e-9, 6.6e-5, o, 1.05e-9, 2e3, 1234};
    ...
    const BCTAlloy SnAg_wtp{
        61'810.62*SnAr, 249.0*SnAr, -3.14, 0.0191, 8.54e-8, 1.82e-9, 1.5e-5, o, 3.07e-10, 2.47e3, 505.1
    };
    ...
    // wt% values.
    const LGKAlloy SucAce_wtp{46'260*SucMr, 1937.5*SucMr, -2.16*SucMr/AceMr, 0.103, 6.62e-8, 1.27e-9, 1.14e-7, o};
}
