...
experiment_paths = [path for path in home_path.glob(pattern='LGK*') if path.is_dir()]
...
fig.suptitle('Sn-3.5Ag Converged Data Points')
...
for i, path in enumerate(experiment_paths):
	...
	# axes[0].annotate('red = R < 0', xy=(-0.4, 0.3), xycoords='axes fraction')
