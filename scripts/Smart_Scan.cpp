// Script used to more robustly newton for higher undercoolings by using the solutions to previous undercoolings as a
...
int main()
{
    ...
    double V0{approx::getV(dT, C0, A)}, R0{approx::getR(dT, C0, A)};

    for (double C0{30}; C0<=50; C0+=10)
        while( (dT<=320) && (dTStep>=std::pow(2, -100)) )
        {
            solvers::Result result{solvers::newton<models::LGK>(dT, C0, A, V0, R0)};
